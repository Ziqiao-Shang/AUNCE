...
def get_dataloader(conf_graph_au):
    ...
    if conf_graph_au.dataset == 'BP4D':
        if conf_graph_au.info_nce == None:
            trainset = BP4D(root_path=conf_graph_au.dataset_path, train = True, if_200 = conf_graph_au.if_200, info_nce = conf_graph_au.info_nce, 
            ...
            valset = BP4D(root_path=conf_graph_au.dataset_path, train = False, if_200 = conf_graph_au.if_200, info_nce = conf_graph_au.info_nce, 
        ...
        if conf_graph_au.info_nce == None:
            trainset = DISFA(root_path=conf_graph_au.dataset_path, train = True, if_200 = conf_graph_au.if_200, info_nce = conf_graph_au.info_nce, 
                            fold = conf_graph_au.fold, transform = image_train_enhance(crop_size=conf_graph_au.crop_size), target_transform = None,  
                            crop_size = conf_graph_au.crop_size)
            train_loader = DataLoader(trainset, batch_size = conf_graph_au.batch_size, shuffle = True, num_workers = conf_graph_au.num_workers, pin_memory=True)
            valset = DISFA(root_path=conf_graph_au.dataset_path, train = False, if_200 = conf_graph_au.if_200, info_nce = conf_graph_au.info_nce, 
                          fold = conf_graph_au.fold, transform = image_test(crop_size=conf_graph_au.crop_size), target_transform = None,
                          crop_size = conf_graph_au.crop_size)
            val_loader = DataLoader(valset, batch_size = conf_graph_au.batch_size, shuffle = False, num_workers = conf_graph_au.num_workers, pin_memory=True)
            return train_loader, val_loader, len(trainset), len(valset)
...
def train_graphau(conf_graph_au, net, train_loader, optimizer, epoch, train_weight, lr, lr_scheduler):       
    net.train()
    losses = AverageMeter() 
    for batch_idx, (inputs, labels) in enumerate(tqdm(train_loader)):
        optimizer = lr_scheduler(param_lr = lr, optimizer = optimizer, epoch = epoch, gamma = conf_graph_au.gamma, stepsize = conf_graph_au.stepsize)
...
def train_graphau_nce(conf_graph_au, net, train_loader, optimizer, epoch, train_weight, lr, lr_scheduler):      
    net.train()
    losses = AverageMeter() 
    for batch_idx, (inputs, inputs_e, labels) in enumerate(tqdm(train_loader)):
        ...
        loss = au_infonce_loss(outputs, outputs_e, labels, train_weight, probability=conf_graph_au.probability)
        loss.backward()
        optimizer.step()
        losses.update(loss.data.item(), inputs.size(0))
        if batch_idx % 10 == 0:
...
def val_graphau(conf_graph_au, net, val_loader, test_weight):
    net.eval()
    losses = AverageMeter()
...
def main(conf_graph_au):
    ...
    train_weight = torch.from_numpy(np.loadtxt(os.path.join(conf_graph_au.dataset_path, 'list_256x256', conf_graph_au.dataset+'_train_weight_fold' + str(conf_graph_au.fold)+'.txt'))).cuda()
    test_weight = torch.from_numpy(np.loadtxt(os.path.join(conf_graph_au.dataset_path, 'list_256x256', conf_graph_au.dataset+'_test_weight_fold' + str(conf_graph_au.fold)+'.txt'))).cuda()
    ...
    if conf_graph_au.optimizer_type == 'SGD':
        optimizer = optim.SGD(net.parameters(), lr=conf_graph_au.init_lr, momentum = conf_graph_au.momentum, weight_decay = conf_graph_au.weight_decay, nesterov = conf_graph_au.use_nesterov) 
    elif conf_graph_au.optimizer_type == 'AdamW':
        optimizer = optim.AdamW(net.parameters(), lr=conf_graph_au.init_lr, betas=(0.9, 0.999), weight_decay = conf_graph_au.weight_decay)
    lr_scheduler = schedule_dict[conf_graph_au.lr_type] 
    ...
    for epoch in range(conf_graph_au.start_epoch + 1, conf_graph_au.n_epochs + 1): 
        ...
        if conf_graph_au.info_nce == None: 
            train_loss = train_graphau(conf_graph_au, net, train_loader, optimizer, epoch, train_weight, lr, lr_scheduler)
            val_loss, val_mean_f1_score, val_f1_score, val_mean_acc, val_acc = val_graphau(conf_graph_au, net, val_loader, test_weight)
            ...
            train_loss = train_graphau_nce(conf_graph_au, net, train_loader, optimizer, epoch, train_weight, lr, lr_scheduler)
...
if __name__=="__main__":
    conf_graph_au = get_config_graphau()
