...
def au_infonce_loss(input, input_e, target, weight, temperature = 0.5, probability = True, size_average=True, reduce=True): 
    ...
    neg_scores = (relation_neg * scores ** 1.2).sum(dim=-2) * occur_labels + (relation_neg * scores ** 0.8).sum(dim=-2) * unoccur_labels
